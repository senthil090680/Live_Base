...
class PS_Pagination {
	...
	function PS_Pagination($connection, $sql, $rows_per_page = 5, $links_per_page = 5) {
	...
	if(!empty($_REQUEST["qid"])){ $qid="&qid=".$_REQUEST["qid"]; }
		if($this->page == 1) {
			return $tag;
		}
		else {
			return '<a href="'.$this->php_self.'?page=1'.$qid.'" class="blink"> '.$tag.'</a>';
		}
	}
	...
	if(!empty($_REQUEST["qid"])){ $qid="&qid=".$_REQUEST["qid"]; }
		...
		if($this->page == $this->max_pages) {
			...
			return '<a href="'.$this->php_self.'?page='.$this->max_pages.$qid.'" class="blink">'.$tag.'</a>';
		}
	}
	...
	if(!empty($_REQUEST["qid"])){ $qid="&qid=".$_REQUEST["qid"]; }
		...
		if($this->page < $this->max_pages) {
			return '<a href="'.$this->php_self.'?page='.($this->page+1).$qid.'" class="blink">'.$tag.'</a>';
		}
		else {
			return $tag;
		}
	...
	if(!empty($_REQUEST["qid"])){ $qid="&qid=".$_REQUEST["qid"]; }
		...
		for( $i=$start ; $i<=$end ; $i++) {
			if($i == $this->page) {
				...
				$links .= ' <a href="'.$this->php_self.'?page='.$i.$qid.'" class="blink">'.$i.'</a> ';
			}
		}
