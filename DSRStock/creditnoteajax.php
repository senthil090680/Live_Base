...
if(isset($return_id) && isset($return_id)) {
	 $where					=	"WHERE Transaction_Number ='$return_id'";
}
?>
<?php
$query_track		=	"SELECT * FROM transaction_return_line $where";
...
if($sortorder == "")
{
	$orderby	=	"ORDER BY Transaction_Line_Number ASC";
} else {
...
$query_salesdate			=	"SELECT * FROM transaction_hdr $where";
...
if($num_rows > 0) {
   ...
   while($row_track	=	mysql_fetch_array($results_dsr)) { ?>		
		<tr>
			<td align='left'><?php
			$prod_nameval	=	getdbval($row_track[Product_code],'Product_description1','Product_code','product');
		    $prod_idval		=	getdbval($row_track[Product_code],'id','Product_code','product');
			if($prod_nameval	== '') {
				$prod_nameval	=	getdbval($row_track[Product_code],'Product_description1','Product_code','customertype_product');
				$prod_idval		=	getdbval($row_track[Product_code],'Product_id','Product_code','customertype_product');
			}
			/*$query_prodname			=	"SELECT Product_description1,id FROM product WHERE Product_code = '$row_track[Product_code]'";		
			...
			echo $prod_nameval; ?>
			<input type="hidden" name="prodcode_<?php echo $r; ?>" id="prodcode_<?php echo $r; ?>" value="<?php echo $prod_idval; ?>" />
			</td>
			<td align='center'><?php echo $row_track[Reurn_quantity]; ?><input type="hidden" name="returnqty_<?php echo $r; ?>" id="returnqty_<?php echo $r; ?>" value="<?php echo $row_track[Reurn_quantity]; ?>" /></td>
			<?php
			$query_priceval			=	"SELECT Price FROM price_master WHERE Product_code = '$row_track[Product_code]'";			
			$res_priceval			=	mysql_query($query_priceval) or die(mysql_error());
			$row_priceval			=	mysql_fetch_array($res_priceval);
			$priceval				=	$row_priceval['Price']; ?>
			<td align='center'><input type="text" onBlur="changetotalval('<?php echo $num_rows; ?>');" name="priceval_<?php echo $r; ?>" id="priceval_<?php echo $r; ?>" value="<?php echo $priceval; ?>" /></td>
			<td align='center'><span id="valuevalspan_<?php echo $r; ?>" ><?php //echo $CustomerName; 
			$totpriceval	+=	$priceval * $row_track[Reurn_quantity];
			echo $valueval	=	$priceval * $row_track[Reurn_quantity];
