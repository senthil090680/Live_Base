...
<table width="100%" align="left" >
 <tr>
  <td style="color:#000;font-size:14px;" align="right">
	<!-- <b>Search :</b> --> <input type='text' name='searchval' id='searchval' />&nbsp;&nbsp;<input type="button" name="Search" value="Search" class="buttons" onclick="javascript:return searchProductShow();"/>
   </td>
    <td style="color:#000; font-size:14px;padding-left:600px;" align="left">
...
<tr>
  ...
  <tbody id="productsadded">
	...
	$sel_supp		=	"SELECT Product_code,Product_description from opening_stock_update WHERE (TransactionQty != '' AND BalanceQty != '' AND TransactionNo != '' AND TransactionType !='') GROUP BY Product_description";
	...
	<?php } // if loop 
		...
		<tr>
			...
			<td align='left' width="41%"  ><input type='hidden' value='<?php echo $row_supp[Product_description]; ?>' name='product_codename_<?php echo $t; ?>' id='product_codename_<?php echo $t; ?>' /><span id="prodnamebg_<?php echo $t; ?>" > <?php echo $row_supp[Product_description]; ?></span></td>

			<td align='center' width="14%">
			<input type='hidden' value='<?php echo $UOM_dis; ?>' name='product_UOM_<?php echo $t; ?>' />

			<input type='text' value='' style="width:50px;text-align:right;" onBlur="changeToPcs('<?php echo $t; ?>','<?php echo $perCartonPrice; ?>',this.value,'<?php echo $uom_coversion; ?>');" name='UOM_cartons_<?php echo $t; ?>' id='UOM_cartons_<?php echo $t; ?>' />
			...
			<td align='center' width="14%"><input type='text' style="width:50px;text-align:right;" onBlur="changeToCartons('<?php echo $t; ?>','<?php echo $perCartonPrice; ?>',this.value,'<?php echo $uom_coversion; ?>');" value='<?php echo $Loaded_Qty; ?>' autocomplete='off' name='Loaded_Qty_<?php echo $t; ?>' id='Loaded_Qty_<?php echo $t; ?>' /></td>

			<td align='right' width="14%">			
			<!-- <input type='hidden' readonly value='<?php echo $Confirmed_Qty; ?>' name='Confirmed_Qty_<?php echo $t; ?>' /> -->
			</td>

			<td align='right' width="14%"><input type='hidden' value='<?php echo $perCartonPrice; ?>' name='price_carton_<?php echo $t; ?>' id='price_carton_<?php echo $t; ?>' /><?php echo number_format($perCartonPrice,2); ?></td>
		
			<td align='right' width="14%"><input type='hidden' value='<?php $total_price_val	=	($Loaded_Qty * $price_val); echo $total_price_val; ?>' name='total_price_value_<?php echo $t; ?>' id='total_price_value_<?php echo $t; ?>' /><span id="value_<?php echo $t; ?>"><?php echo number_format($total_price_val,2); ?></span></td>
...
<div id="loadergif" class="loadingstyle"><img src="../images/bigload.gif" /> </div>
<input type="button" name="submitval" id="submitval" class="buttons" value="Confirm" onClick="return checkdailystock();" /><input type="hidden" name="submithidden" id="submithidden" class="buttons" value="Save" />

<!-- <input type="button" class="buttons" onClick="checkprodconfirm();" value="Save" /> -->

<span id="spannbsp">&nbsp;&nbsp;&nbsp;&nbsp;</span><input type="button" name="cancel" id="cancelbutt" value="Cancel" class="buttons" onclick="javascript:return closeProductShow();"/>&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="buttons" onClick="printprodconfirm();" value="View" />
	 </p>
