...
if(isset($_REQUEST[DateVal]) && $_REQUEST[DateVal] !='') {
	$nextrecval			=	"WHERE Date LIKE '%$DateVal%' AND dsr_id = '$DSR_Code' ORDER BY id DESC";
} else {
	$nextrecval		=	"";
}
$where		=	"$nextrecval";
...
$sel_cusdet					=	"SELECT id,Customer_Name,sequence_number,AddressLine1,AddressLine2,lga,City,contactperson,contactnumber from customer WHERE route = '$route_code' AND route != ''";
$res_cusdet					=	mysql_query($sel_cusdet) or die(mysql_error());	
//echo $nor_dsr					=	mysql_num_rows($res_cusdet);
//exit;
...
<table width="100%" align="left" id="productsadd">
 <tr>
  <td>
  <div <?php if($nor_dsr == 0) { ?> class="condailynorec" <?php } else { ?> class="con_str" <?php } ?> >
  <table width="100%">
   <thead>
    <tr>
...
<div class="paginationfile" align="center">
	  ...
	  <span id="printopen" style="padding-left:20px;padding-top:10px;<?php if($nor_dsr > 0 ) { echo "display:block;"; } else { echo "display:none;"; } ?>" ><input type="button" name="kdproduct" value="Print" class="buttons" onclick="print_pages('printcusconajax');"></span>
	<form id="printcusconajax" target="_blank" action="printcusconajax.php" method="post">
