  ...
  <tbody id="productsadded" style="font-weight:lighter;">
	...
	if($num_rows >0) {
		while($fetch				=	mysql_fetch_array($results_dsr)) { 
			...
			?>
		<tr>
	<td align="center" style="width:2%"><?php echo $fetch['Transaction_Line_Number'];?></td>
	...
	<td style="width:5%"><table border="0" width="100%">
			<tr>
				<td align="right"><?php if($fetch['Order_quantity'] != '') { echo $fetch['Order_quantity']; } else { echo "0"; } ?></td><td align="right"><?php if($fetch['Sold_quantity'] != '') { echo $fetch['Sold_quantity']; } else { echo "0"; } ?></td><td align="right"><?php if($Product_Scheme_Flag == 1 && $Scheme_Flag == 0) { echo "1"; } else { echo "0"; } ?></td>
			</tr>
			</table></td>
	<td align="right" style="width:3%"><?php echo number_format($priceval,2); ?></td>
	<td align="right" style="width:3%"><?php echo number_format($valueval,2); ?></td>
	</tr>
	<?php 	} //while loop
...
<div style="clear:both"></div>
<span id="printopen" style="padding-left:600px;padding-top:10px;<?php if($num_rows > 0 ) { echo "display:block;"; } else { echo "display:none;"; } ?>" ><input type="button" name="kdproduct" value="Print" class="buttons" onclick="hideprintbutton();"></span>
<?php exit(0);?>