...
if(isset($_POST[submit]) && $_POST[submit] == 'Save') {
	...
	header("location:creditnoteview.php?no=1");
}
?>
<!------------------------------- Form -------------------------------------------------->
<style type="text/css">
		...
		<div class="concredit">
			...
			<table align="center" width="100%">
				...
				<tbody>
					...
					<tr>
						<td align="center">
							<strong>SALES RETURN TRANSACTION NO : 		
								<select name="salestransno" id="salestransno" onChange="bringreturntrans(this.value);" >
									<option value="">--Select--</option>
									<?php
										$query_salesreturn			=	"SELECT * FROM transaction_return_line GROUP BY Transaction_Number";			
										$res_salesreturn			=	mysql_query($query_salesreturn) or die(mysql_error());
										$rowcnt_salesreturn			=	mysql_num_rows($res_salesreturn);
										
										if($rowcnt_salesreturn > 0 ) {
											while($row_salesreturn				=	mysql_fetch_array($res_salesreturn)) {
												$Transaction_Number				=	$row_salesreturn['Transaction_Number'];
												$query_creditcheck				=	"SELECT * FROM credit_note WHERE return_transno = '$Transaction_Number'";	
												$res_creditcheck				=	mysql_query($query_creditcheck) or die(mysql_error());
												$rowcnt_creditcheck				=	mysql_num_rows($res_creditcheck);
												
												if($rowcnt_creditcheck == 0 ) { ?>
													<option value="<?php echo $Transaction_Number; ?>" ><?php echo $Transaction_Number; ?></option>
												<?php } 							
											} // while loop
										} // row count loop ?>
								</select>
		...
		<table width="50%" style="clear:both">
			<tr align="center" height="50px;">
				...
				<!-- <input type="button" name="submit" id="submit" class="buttons" value="Save" onClick="return creditnotesave();" />&nbsp;&nbsp;&nbsp;&nbsp; -->
					...
					<input type="reset" name="reset" class="buttons" value="Clear" id="clear" onclick="return cusclr();"/>&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="button" name="cancel" value="Cancel" class="buttons" onclick="window.location='../include/menu.php'"/>&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="button" name="view" value="View" class="buttons" onclick="window.location='creditnoteview.php'"/>
				</td>
			</tr>
		</table> 
