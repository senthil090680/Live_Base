...
if($id=='') {
	if(isset($_POST['submit'])) {
		...
		$query_devid				=	"SELECT id FROM device_master WHERE device_description = '$device_name'";			
		...
		if($dsr_name=='' || $dsr_code==''  || $device_name=='' || $routes=='' || $location ==''|| $vehicles ==''|| $cycle =='') {
			...
			$ass_sql					=	"INSERT INTO cycle_assignment SET KD_Code='$KD_Code',flag_status='$cycle',dsr_id='$dsrid',dsr_code='$dsr_code',device_id='$devid',device_code='$device_code',route_id='$routeid',location_id='$locid',vehicle_id='$vehid',Date=NOW(),end_flag_status='0',assign_id='$last_inserted_id'";
			//exit;
...
function devicecode()
{
	var val=$('#devicename option:selected').text();
	 $.ajax({
            url: 'get_devicecode.php?val=' + val,
...
<tr>
    ...
    <td><select name="devicename" id="devicename" onchange="return devicecode();">
      <option value="">---Select---</option>
