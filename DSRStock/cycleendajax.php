...
while($row_productcode		=	mysql_fetch_array($res_productcode)) {
	$productcode_ds[]		=	ltrim($row_productcode['Product_code'],' ');
...
if($rowcnt_prodhdr > 0 ) {
	while($row_prodhdr					=	mysql_fetch_array($res_prodhdr)) {
		...
		while($row_prodlist				=	mysql_fetch_array($res_prodlist)) {
			$productcode_hdr[]			=	ltrim($row_prodlist[Product_code],' ');
...
if($rowcnt_prodhdrret > 0 ) {
	while($row_prodhdrret				=	mysql_fetch_array($res_prodhdrret)) {
		...
		while($row_prodlistret			=	mysql_fetch_array($res_prodlistret)) {
			$productcode_hdrret[]		=	ltrim($row_prodlistret[Product_code],' ');
...
if($rowcnt_return_trno > 0 ) {
	while($row_return_trno			=	mysql_fetch_array($res_return_trno)) {
		$row_return_trno_arr[]		=	$row_return_trno;
	}
}
//pre($row_return_trno_arr);
//exit;
...
//exit;
	...
	if(!empty($num_rows)){
		...
		while($fetch = mysql_fetch_array($results)) {
			...
			$productcode_str				=	implode("','",$productcode);
			...
			if($productcode != '' && !empty($productcode)) {
				...
				/*$query_for_product				=	"SELECT id,Product_code,Product_description1 FROM product WHERE Product_code IN ('$productcode_str')";	
				$res_for_product				=	mysql_query($query_for_product) or die(mysql_error());
				$rowcnt_for_product				=	mysql_num_rows($res_for_product);
				
				while($row_fp					=	mysql_fetch_array($res_for_product)) {
					$P_code						=	$row_fp['Product_code'];
					$prodname					=	$row_fp['Product_description1']; */
				$m								=	0;
				//count($productcode);
				foreach($productcode as $P_code) {
					...
					if($rowcnt_cancel > 0 ) {
						foreach($row_cancel_arr		AS $row_cancel_val) {
						//while($row_cancel			=	mysql_fetch_array($res_cancel)) {
							$Transaction_Number			=	$row_cancel_val[Transaction_Number];
							$Transaction_type			=	$row_cancel_val[Transaction_type];
					...
					/* commented as it is made at top as single time instead of multiple time in the query this is php optimization 
					...
					*/
					...
					if($rowcnt_return_trno > 0 ) {
						//while($row_return_trno			=	mysql_fetch_array($res_return_trno)) {
						foreach($row_return_trno_arr	AS $res_return_trno_val) {
							$Transaction_Number			=	$res_return_trno_val[Transaction_Number];
							$Transaction_type			=	$res_return_trno_val[Transaction_type];
					...
					<td align="right"><input type="hidden" readonly name="short_reason_<?php echo $P_code; ?>" id="short_reason_<?php echo $P_code; ?>" value="" /><input type="hidden" readonly name="quantity_shortage_<?php echo $P_code; ?>" id="quantity_shortage_<?php echo $P_code; ?>" value="" /><input type="hidden" name="prod_name_<?php echo $P_code; ?>" id="prod_name_<?php echo $P_code; ?>" value="<?php echo $prodname; ?>" /> <span id="qtyshotspan_<?php echo $P_code; ?>"></span></td>
...
<?php
	...
	$sel_amtdep					=	"SELECT SUM(Amount_Deposited) AS AMT_DEP from dsr_collection WHERE DSR_Code = '$DSR_Code' AND (Date >= '$cycle_start_date' AND Date <= '$cycle_end_date') ORDER BY Date";
	$res_amtdep					=	mysql_query($sel_amtdep) or die(mysql_error());	
	$row_amtdep					=	mysql_fetch_array($res_amtdep);
	$Total_Amount_Deposited		=	$row_amtdep['AMT_DEP'];

	/*while($row_amtdep				=	mysql_fetch_array($res_amtdep)) {
		$Total_Amount_Deposited		+=	$row_amtdep['Amount_Deposited'];
	}*/
	$sel_salevalue				=	"SELECT SUM(total_sale_value) AS TOT_SAL from sale_and_collection WHERE DSR_Code = '$DSR_Code' AND (Date >= '$cycle_start_date' AND Date <= '$cycle_end_date') ORDER BY Date";
	//exit;
	$res_salevalue				=	mysql_query($sel_salevalue) or die(mysql_error());	
	$row_salevalue				=	mysql_fetch_array($res_salevalue);
	$Total_Sale_Value			=	$row_salevalue['TOT_SAL'];
