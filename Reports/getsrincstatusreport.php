...
if(isset($_REQUEST[propmonths]) && $_REQUEST[propmonths] !='') {
	...
	$query_trans									=   "SELECT DISTINCT KD_Code,DSR_Code,Date,SUM(visit_Count) AS VIS_CNT,SUM(Invoice_Count) AS INV_CNT,SUM(effective_count) AS EFF_CNT,SUM(productive_count) AS PROD_CNT,SUM(Invoice_Line_Count) AS INV_LIN_CNT,Total_Sale_Value,Drop_Size_Value,Basket_Size_Value FROM dsr_metrics $complete_query GROUP BY DSR_Code ORDER BY Date";
	...
	foreach($finalSearchInfo as $val_kd){
		...
		if($kdInfo[$val_kd["KD_Code"]]["KD_Code"] == $val_kd[KD_Code]) {                                     
			...
			$finalkdInfo[$i]["visit_Count"]							=   $val_kd["VIS_CNT"];
			$finalkdInfo[$i]["SALES_Count"]							=   $val_kd["INV_CNT"];
			//$finalkdInfo[$i]["EFF_CNT"]							=   $val_kd["EFF_CNT"];
			//$finalkdInfo[$i]["PROD_CNT"]							=   $val_kd["PROD_CNT"];

			$finalkdInfo[$i]["EFF_CNT"]								=   tofindmetviscntformonth($propyears,$monthval,$finalkdInfo[$i]["DSRCode"],'effective_count');
			$finalkdInfo[$i]["PROD_CNT"]								=   tofindmetviscntformonth($propyears,$monthval,$finalkdInfo[$i]["DSRCode"],'productive_count');

			$finalkdInfo[$i]["Invoice_Line_Count"]					=   $val_kd["INV_LIN_CNT"];
...
?>
	 <tr>
		 ...
		 <td><?php if($SearchVal["visit_Count"] != '') { echo $SearchVal["visit_Count"]; } else {  echo '0';  } ?></td>
		 <td><?php if($SearchVal["SALES_Count"] != '') { echo $SearchVal["SALES_Count"]; } else {  echo '0';  } ?></td>	
		 <td><?php if($SearchVal["EFF_COV"] != '') { echo $SearchVal["EFF_COV"]; } else {  echo '0';  } ?></td>
		 <td <?php 
...
?>
	 <tr>
		 ...
		 <td><?php if($SearchVal["visit_Count"] != '') { echo $SearchVal["visit_Count"]; } else { echo '0'; } ?></td>
		 <td><?php if($SearchVal["SALES_Count"] != '') { echo $SearchVal["SALES_Count"]; } else { echo '0';  }?></td>	
		 <td><?php if($SearchVal["COV_COV"] != '') { echo $SearchVal["COV_COV"]; } else { echo '0'; } ?></td>
		 <td <?php 
...
<?php }
 ?>
	...
	//if($e != 0) { ?>
		...
		?>
			 <tr>
				 ...
				 <td><?php if($SearchVal["visit_Count"] != '' ) { echo $SearchVal["visit_Count"]; } else { echo '0'; } ?></td>
				 <td><?php if($SearchVal["SALES_Count"] != '' ) { echo $SearchVal["SALES_Count"]; } else { echo '0'; } ?></td>
				 <td><?php if($SearchVal["PRO_COV"] != '' ) { echo $SearchVal["PRO_COV"]; } else { echo '0'; } ?> </td>
				 <td <?php 
				 ...
				 ?> ><?php if($SearchVal["POS_INC_PRO"] != '') { echo number_format($SearchVal["POS_INC_PRO"],2); } else { echo '0.00'; } ?></td>	
				 <td><?php if($SearchVal["PAY_INC_PRO"] != '') { echo number_format($SearchVal["PAY_INC_PRO"],2); } else { echo '0.00'; } ?></td>	
			 </tr>	
		<?php }
		} else { ?>
			<tr>
	...
	<tr>
	 ...
	 <td align='right'><a href="javascript:void(0);" onclick="showfullproducts('<?php echo $DSRCODE_PROD; ?>','<?php echo $propmonths; ?>','<?php echo $propyears; ?>');"><?php if($SearchVal["SUM_QTY"] != '') { echo number_format($SearchVal["SUM_QTY"]); } else { echo '0'; } ?></a>
...
echo "~";
	...
	<tr>
	 ...
	 <td align='right'><a href="javascript:void(0);" onclick="showfullbrands('<?php echo $DSRCODE_BRAND; ?>','<?php echo $propmonths; ?>','<?php echo $propyears; ?>');"><?php if($SearchVal["SUM_QTY"] != '') { echo number_format($SearchVal["SUM_QTY"]); } else { echo '0'; } ?></a></td>
	 <td <?php 
		 if($SearchVal["SRINCTGTFLAG"] == '0') {
			if($SearchVal["SUM_QTY"] >= $SearchVal["TGT_UNT"]) {
				?>style="color:#008000;" <?php 
	...
	<?php echo "~"; if(count($finalSearchInfo) > 0) { ?>

		<span id="printopen" style="padding-left:0px;padding-top:10px;<?php if($finalSearchInfo > 0 ) { echo "display:block;"; } else { echo "display:none;"; } ?>" ><input type="button" name="kdproduct" value="Print" class="buttons" onclick="print_pages('printsrincstatusajax');"></span>
