...
if(isset($_REQUEST[propmonths]) && $_REQUEST[propmonths] !='') {
	...
	if($custype == '' || $custype == 'null') {
		...
		if(is_array($custype)) {
			$custypeprint		=	$custypeval;
		} else {
			$custypeprint		=	$custype;		
		}
	}
...
<span id="printopen" style="padding-left:470px;padding-top:10px;<?php if($num_rows > 0 ) { echo "display:block;"; } else { echo "display:none;"; } ?>" ><input type="button" name="kdproduct" value="Print" class="buttons" onclick="print_pages('printposmcovajax');"></span>
<form id="printposmcovajax" target="_blank" action="printposmcovajax.php" method="post">
	...
	<input type="hidden" name="custype" id="custype" value="<?php echo $custypeprint; ?>" />
</form>
<?php } else { ?>
 <tr>
