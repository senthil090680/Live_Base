...
if(isset($_REQUEST[propmonths]) && $_REQUEST[propmonths] !='') {
	...
	if($srcode	==	'') {
		...
		$DSR_Codestr		=	$srcode;
		$datecol		=	"Date LIKE '".$datecolvalue."%' AND DSR_Code = '$srcode' ";
	...
	foreach($checktimeInfo AS $checktime) {		
		...
		if($p != 1) {			
			if($checkdatedsr  != $combinedatedsr) {				
				...
				echo $outtime[$checkdatedsr][TOTAL]."<br>";
				echo $intime[$combinedatedsr][TOTAL]."<br>";
				echo $timecal[$combinedatedsr][TOTAL]."<br>";*/
				$timecal[$combinedatedsr][TOTAL]			=	getTimeDiffHours($timecal[$combinedatedsr][FIRSTIN],$timecal[$combinedatedsr][LASTOUT]);
...
//KD_Code,DSR_Code,Date,SUM(visit_Count) AS ACTVISCNT,SUM(Invoice_Count) AS ACTINVCNT,SUM(effective_count) AS EFFCNT,SUM(productive_count) AS PROCNT,SUM(Invoice_Line_Count) ACTLINCNT,SUM(Total_Sale_Value) AS ACTSALTOT,SUM(Drop_Size_Value) AS ACTDROP,SUM(Basket_Size_Value) AS ACTBAS
	...
	foreach($finalSearchInfo as $val_checkin){
		...
		if($timecal[$val_checkin["DSR_Code"]]["DSR_CODE"] == $valDSRCHC) { 
			...
			$finalcheckinInfo[$i]["TOTCUS"]							=   tofindtotplancustformonth($propyears,$propmonths,$finalcheckinInfo[$i]["DSRCode"]);
			...
			$finalcheckinInfo[$i]["EFFCNT"]								=   tofindmetviscntformonth($propyears,$propmonths,$finalcheckinInfo[$i]["DSRCode"],'effective_count');
			$finalcheckinInfo[$i]["PROCNT"]								=   tofindmetviscntformonth($propyears,$propmonths,$finalcheckinInfo[$i]["DSRCode"],'productive_count');

			$finalcheckinInfo[$i]["EFF_COV"]								=   ceil($finalSearchInfo[$i]["PROCNT"]/$finalSearchInfo[$i]["ACTVISCNT"]);
			$finalcheckinInfo[$i]["PRO_COV"]								=   ceil($finalSearchInfo[$i]["ACTINVCNT"]/$finalSearchInfo[$i]["ACTVISCNT"]);

			$finalcheckinInfo[$i]["ACTDROP"]							=   round($finalcheckinInfo[$i]["ACTSALTOT"]/$finalcheckinInfo[$i]["PROCNT"]);
			$finalcheckinInfo[$i]["ACTBAS"]								=   round($finalcheckinInfo[$i]["ACTSALTOT"]/$val_checkin["ACTLINCNT"]);
	...
	$rowcnt_tgtval								=   mysql_num_rows($res_tgtval);
	...
	if($rowcnt_tgtval > 0 ) {
		...
		*/
		...
		foreach($finalSearchInfo as $val_merge){
			...
			$finaltgtmergeInfo[$i]["PRO_COV"]							=   $val_merge["PRO_COV"];
			$finaltgtmergeInfo[$i]["EFF_COV"]							=   $val_merge["EFF_COV"];
			$finaltgtmergeInfo[$i]["ACH_PER"]							=   ceil(($finaltgtmergeInfo[$i]["ACTSALTOT"]/$finaltgtmergeInfo[$i]["TOTAL_TGT_NAIRA"])*(100));
		...
		$i=0;
		$SALQTYVAL				=	multi_array_sum($transTotalProdInfo,'SALQTY');
		foreach($finalSearchInfo as $val_merge){
			$finalwithnotgtInfo[$i]["DSRCode"]							=   $val_merge["DSRCode"];
			$finalwithnotgtInfo[$i]["ACTVISCNT"]						=   $val_merge["ACTVISCNT"];
			$finalwithnotgtInfo[$i]["ACTINVCNT"]						=   $val_merge["ACTINVCNT"];
			$finalwithnotgtInfo[$i]["EFFCNT"]							=   $val_merge["EFFCNT"];
			$finalwithnotgtInfo[$i]["PROCNT"]							=   $val_merge["PROCNT"];
			$finalwithnotgtInfo[$i]["ACTLINCNT"]						=   $val_merge["ACTLINCNT"];
			$finalwithnotgtInfo[$i]["ACTSALTOT"]						=   $val_merge["ACTSALTOT"];
			$finalwithnotgtInfo[$i]["ACTDROP"]							=   $val_merge["ACTDROP"];
			$finalwithnotgtInfo[$i]["ACTBAS"]							=   $val_merge["ACTBAS"];
			$finalwithnotgtInfo[$i]["TOTCUS"]							=   $val_merge["TOTCUS"];
			$finalwithnotgtInfo[$i]["TOTALHRS"]							=   $val_merge["TOTALHRS"];

			$finaltgtmergeInfo[$i]["TOTAL_TGT_NAIRA"]					=   0;
			$finaltgtmergeInfo[$i]["TOTAL_ACT_NAIRA"]					=   0;

			$finalwithnotgtInfo[$i]["PRO_COV"]							=   $val_merge["PRO_COV"];
			$finalwithnotgtInfo[$i]["EFF_COV"]							=   $val_merge["EFF_COV"];

			$finalwithnotgtInfo[$i]["ACH_PER"]							=   0;

			$finalwithnotgtInfo[$i]["SALQTY"]							=   $SALQTYVAL;
			$i++;
		}
		$finalSearchInfo			=	$finalwithnotgtInfo;
	}

	//pre($finalSearchInfo);
	//exit;

	$query_focus										=   "SELECT KD_Code,DSR_Code,Product_code,Transaction_Number,SUM(Focus_Flag) AS TOTFOCLIN, COUNT(DISTINCT(Product_code)) AS TOTFOCPRO FROM transaction_line WHERE Transaction_Number IN ('".$transno_Total."') $prodcodecol AND (Focus_Flag = 1 OR Focus_Flag = '1') AND COUNT(DISTINCT(Product_code)) > 0 ORDER BY Product_code";
	//echo $query_focus;
	//exit;
	...
	$rowcnt_focus										=   mysql_num_rows($res_focus);
	//echo $rowcnt_focus;
	//exit;
	...
	if($rowcnt_focus > 0) {
		...
		foreach($finalSearchInfo as $val_focus){
			...
			if($transFocusInfo[$val_focus["DSRCode"]]["DSR_Code"] == $val_focus["DSRCode"]) {
				...
				$finalfocusInfo[$i]["PRO_COV"]								=   $val_focus["PRO_COV"];
				$finalfocusInfo[$i]["EFF_COV"]								=   $val_focus["EFF_COV"];
				$finalfocusInfo[$i]["ACH_PER"]								=   $val_focus["ACH_PER"];
	...
	$query_focusact										=   "SELECT DSR_Code,SUM(focus_Flag) AS FOCUS_ACT FROM dailystockloading $complete_query AND (focus_Flag ='1') AND SUM(focus_Flag) > 0";
	//echo $query_focusact;
	//exit;
	$res_focusact										=   mysql_query($query_focusact);
	$rowcnt_focusact									=   mysql_num_rows($res_focusact);

	if($rowcnt_focusact > 0 ){
		...
		foreach($finalSearchInfo as $val_focusact){
			...
			if($focusactInfo[$val_focusact["DSRCode"]]["DSR_Code"] == $val_focusact["DSRCode"]) {
				...
				$finalfocusInfo[$i]["PRO_COV"]								=   $val_focusact["PRO_COV"];
				$finalfocusInfo[$i]["EFF_COV"]								=   $val_focusact["EFF_COV"];
				$finalfocusInfo[$i]["ACH_PER"]								=   $val_focusact["ACH_PER"];
				...
				$finalfocusInfo[$i]["PRO_COV"]								=   $val_focusact["PRO_COV"];				$finalfocusInfo[$i]["EFF_COV"]								=   $val_focusact["EFF_COV"];				$finalfocusInfo[$i]["ACH_PER"]								=   $val_focusact["ACH_PER"];
	...
	$rowcnt_tgtsetval													=   mysql_num_rows($res_tgtsetval);

	if($rowcnt_tgtsetval > 0 ) {
...
if($arrcnt > 0) {
	...
	?>	 
	  ...
	  <tr>
		<th align="center" colspan="19">SR METRICS</th>
	 </tr>
     <tr>
		<th align="left" colspan="19"><?php echo "Month & Year : &nbsp;&nbsp;&nbsp;".date('F',mktime(0,0,0,$propmonths))."&nbsp;".$propyears."&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;SR : &nbsp;&nbsp;".upperstate(getdbval($srcode,'DSRName','DSR_Code','dsr')); ?></th>		
	 </tr>

	  <tr>
	    <td>
	  <table border="1" width="100%" style="border-collapse:collapse;"> <!--First Table-->
				<thead>
				  <tr>
			 ...
			 <tr><td width="50%">Target</td><td align="right"><?php if($ResultVal[TOTAL_TGT_NAIRA] != '') { echo number_format($ResultVal[TOTAL_TGT_NAIRA],2); } else { echo "0"; } ?></td></tr>
			 <!-- <tr><td width="50%">Actual</td><td align="right"><?php if($ResultVal[TOTAL_ACT_NAIRA] != '') { echo number_format($ResultVal[TOTAL_ACT_NAIRA],2); } else { echo $ResultVal["SALQTY"]; } ?></td></tr> -->
			 <tr><td width="50%">Actual</td><td align="right"><?php if($ResultVal[ACTSALTOT] != '') { echo number_format($ResultVal[ACTSALTOT],2); } else { echo "0"; } ?></td></tr>
			 <tr><td width="50%">% Achievement</td><td align="right"><?php if($ResultVal[ACH_PER] != '') { echo $ResultVal[ACH_PER]; } else { echo "0"; } ?></td></tr>
			 </table>
			 </td> <!-- Sale Ends-->
			 ...
			 <tr>
			 <td align="right" height="40px"><?php if($ResultVal[TOTCUS] != '') { echo $ResultVal[TOTCUS]; } else { echo "0"; } ?></td>
			 <td align="right" height="40px"><?php if($ResultVal[EFFCNT] != '') { echo $ResultVal[EFFCNT]; } else { echo "0"; } ?></td>
			 <td align="right" height="40px"><?php if($ResultVal[PROCNT] != '') { echo $ResultVal[PROCNT]; } else { echo "0"; } ?></td>
			 <td align="right" height="40px"><?php if($ResultVal[PRO_COV] != '') { echo $ResultVal[PRO_COV]; } else { echo "0"; } ?></td>
			 <td align="right" height="40px"><?php if($ResultVal[EFF_COV] != '') { echo $ResultVal[EFF_COV]; } else { echo "0"; } ?></td>
			 </tr>
			 </table>
			 </td>  <!--customer coverage start-->      
			</tr> <!--First TR End-->
  ...
  <tr>
  <td>
  <table width="100%" border="1" style="border-collapse:collapse;"> <!--Second Table-->
 		<thead>
			  <tr>
		...
		<tbody>
			 ...
			 <tr>
				 <td>Total Sale (Naira)</td>
				 <td align="right"><?php if($ResultVal[ACTSALTOT] != '') { echo number_format($ResultVal[ACTSALTOT],2); } else { echo "0"; } ?></td>
			 </tr>
			 <tr>
				 <td>Total Lines</td>
				 <td align="right"><?php if($ResultVal[ACTLINCNT] != '') { echo number_format($ResultVal[ACTLINCNT]); } else { echo "0"; } ?></td>
			 </tr>


			 <tr>
				 <td>Total Focus Products</td>
				 <td align="right"><?php if($ResultVal[TOTFOCPRO] != '') { echo number_format($ResultVal[TOTFOCPRO]); } else { echo "0"; } ?></td>
			 </tr>
			 <tr>
				 <td>Total Focus Lines</td>
				 <td align="right"><?php if($ResultVal[TOTFOCLIN] != '') { echo number_format($ResultVal[TOTFOCLIN]); } else { echo "0"; } ?></td>
			 </tr>
			 <tr>
				 <td>Total Checked In Hours</td>
				 <td align="right"><?php if($ResultVal[TOTALHRS] != '') { echo $ResultVal[TOTALHRS]; } else { echo "0"; } ?></td>
			 </tr>
		  </table>
		  
			<table width="50%" align="right"><!-- Metric Start Table-->
			 <tr>
			 ...
			 <tr>
				 <td height="75px" align="center"><?php if($ResultVal[ACTDROP] != '') { echo number_format($ResultVal[ACTDROP]); } else { echo "0"; } ?></td>
				 <td height="75px" align="center"><?php if($ResultVal[ACTBAS] != '') { echo number_format($ResultVal[ACTBAS]); } else { echo "0"; } ?></td>
				 <td height="75px" align="center"><?php if($ResultVal[FOCUS_COV] != '') { echo number_format($ResultVal[FOCUS_COV]); } else { echo "0"; } ?></td>
				 <td height="75px" align="center"><?php if($ResultVal[FOCUS_EFF] != '') { echo number_format($ResultVal[FOCUS_EFF]); } else { echo "0"; } ?></td>
			 </tr>
			 </table>
			</tbody>
			  <!-- Metric Ends-->
	...
	<tr>
	  <td>
		   ...
		   <table width="100%" border="1" style="border-collapse:collapse;"> <!--Third Table-->
			<thead>
			  <tr>
			...
			<div >
			 ...
			 <tr>
				 ...
				 <td width="20%" align="center">Customer Coverage
					<table width="100%">
						<tr>
							<td align="center"><b>Planned</b></td>
		...
		foreach($finalAllProdInfo AS $SearchVal) { ?>
			 <tr>
				 <td ><?php echo $SearchVal[CUSTYPVAL]; ?></td>
				 <td >
					<table width="100%">
    ...
    <!-- <script type="text/javascript">
      ...
      function drawChart() {
		...
		<td valign="top">
			<table width="100%"align="right" style="border-collapse:collapse;"><!-- Incentive Start Table-->
			 <tr>
				 <td align="center" width="15%"><strong>ECO INCENTIVE</strong></td>
				 <td align="center" width="10%"><?php if($ResultVal[ACTDROP] != '') { echo number_format($ResultVal[ECO_INC],2); } else { echo "0.00"; } ?></td>
				 <td align="center" width="15%"><strong>QUANTITY INCENTIVE</strong></td>
				 <td align="center" width="10%"><?php if($ResultVal[TOTAL_ACT_NAIRA] != '') { echo number_format($ResultVal[TOTAL_ACT_NAIRA],2); } else { echo "0.00"; } ?></td>
			 </tr>
...
<table border="1" width="100%">
	  <tr>
	    <td>
		  ...
		  <table border="1" width="100%"> <!--First Table-->
			<thead>
			  <tr>
				<th align="center" style="width:50%">Sales(Naira)</th>
				<th align="center" style="width:50%">Customer Coverage</th>
  ...
  <tr>
  <td>
  <table width="100%" style="border-collapse:collapse;" border="1"> <!--Second Table-->
 		<thead>
			  <tr>
				<th align="center" style="width:25%">ITEM</th>
				<th align="center" style="width:25%">VALUE</th>
	...
	<tr>
	  <td>
	   <table width="100%" style="border-collapse:collapse;" border="1"> <!--Third Table-->
 		<thead>
		  <tr>
			<th align="center" style="width:25%">POSM COVERAGE</th>
			<th align="center" style="width:25%">INCENTIVES(NAIRA)</th>
		...
		<td>
        <table width="100%" style="border-collapse:collapse;" align="right"><!-- Incentive Start Table-->
         <tr>
			 <td align="center" width="15%"><strong>ECO INCENTIVE</strong></td>
	...
	<tr>
	  <td>
	   <table width="100%" style="border-collapse:collapse;" border="1"> <!--Third Table-->
		...
		<td>
        <table width="100%" align="right"><!-- Incentive Start Table-->
         <tr>
			 <td align="center" width="15%"><strong>ECO INCENTIVE</strong></td>
			 <td align="center" width="10%">-</td>
			 <td align="center" width="15%"><strong>QUANTITY INCENTIVE</strong></td>
