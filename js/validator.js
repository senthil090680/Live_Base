...
function checkdailystock() {
	...
	//alert('ere');
	var verifystock	=	confirm("Please Verify the Stocks to Proceed");
	var wem			=	50;
	if(verifystock) {
		$("#loadergif").css({"display":"block","fadeIn":"slow"});
		if(wem % 50 == 0) {
			window.setTimeout(function() {
				//alert("33");
				$("#submitval").val("Save");
				$("#submitval").removeAttr("onClick");
				//$("#cancelbutt").remove();
				//$("#spannbsp").remove();
				$("#submitval").attr("onClick","return checksavedailystock()");
...
function confirmcycle() {
	...
	for(var k=0; k < prodlen; k++) {
		...
		KD_Val					=	$('#KD_returned_qty_'+splitprod[k]).val();
		if(KD_Val[0] == '-'){
			$('#KD_returned_qty_'+splitprod[k]).focus();
			$('.myaligncycleajx').html('ERR : Only Positive values!');
			$('#errormsgcycleajx').css('display','block');
			setTimeout(function () {
				$('#errormsgcycleajx').hide();
			},5000);
		...
		if(KD_returned_qty == '' && KD_returned_qty != 0){

			//alert(KD_returned_qty+"eee");
...
function pagination_ajax(page,params) {
	...
	$.ajax({
		...
		type: "get",
		dataType: "text",
		data : { "fromDate" : splitparam[0], "toDate" : splitparam[1], "sortorder" : splitparam[2], "ordercol" : splitparam[3], "Product_name" : splitparam[4], "page" : page },
		success : function(dataval) {
			var actdata		=	$.trim(dataval);
			//alert(actdata);
			var splitval	=	actdata.split('~');
...
function searchstostaajax(page) {
...
function checksrbrincentry(rowcnt,curmonthval,curyearval) {
	var srcode					=	$("#srcode").val();
	var brandcode				=	$("#brandcode").val();
	var frommonth				=	$("#frommonth").val();
	var tomonth					=	$("#tomonth").val();
...
function srbrincviewajax(page,params){   // For pagination and sorting of the cycle assignment view page
	var splitparam		=	params.split("&");
	var DSR_name	=	splitparam[0];
	var sortorder		=	splitparam[1];
	var ordercol		=	splitparam[2];
	$.ajax({
		url : "srbrincentiveviewajax.php",
		type: "get",
		dataType: "text",
		data : { "DSR_name" : DSR_name, "sortorder" : sortorder, "ordercol" : ordercol, "page" : page },
		success : function(dataval) {
...
function searchsrbrincviewajax(page){  // For pagination and sorting of the cycle assignment search in view page
	var DSR_name	=	$("input[name='DSR_name']").val();
	//alert(Product_name);
	$.ajax({
		url : "srbrincentiveviewajax.php",
...
function srincstatus() {
	...
	$.ajax({
		...
		success		:	function(dataval) {
			...
			$("#allbrandsta").html(splitval[4]);
			$("#printbutton").html(splitval[5]);
			//alert(codevalue);
		}
	});
}
...
function getbrandforbrand() {
	...
	if(brandcodes == '' || brandcodes == null) {
		...
		$('#errormsgsrinc').css('display','block');
		$("#brandcode option:selected").attr("selected",false);
		var prodspan				=	getbrandbasedbrand(brandcodes);
		$("#brandbasedbrandspan").html(prodspan);
		$("#brandbasedbrandspan").css("display","block");
...
function getbrandbasedbrand(codeval) {
	codevalue					=		'';
	$.ajax({
		url			:	"getbrandforbrand.php",
		type		:	"get",
		dataType	:	"text",
		async		:	false,
		data		:	{ "codeval" : codeval },
...
function currencyformat(amt,t) {
	...
	//alert(amt.indexOf(","));
	if(amt.indexOf(',') == -1) { // it will return the searched string index, otherwise if not return -1
		amt		=	parseInt(amt);
		amt		=	amt.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,");
	}
	$("#Amount_Deposited_"+t).val(amt);
	//return amtval;
}
...
function searchProductShow() {
	var totalprod	=	$("#prodcnt").val();
	//alert(totalprod);
	var searchval	=	$("#searchval").val();
	searchval		=	searchval.toLowerCase();
	//alert(searchval);
	var w			=	0;
	for(var k = 1; k <= totalprod; k++) {		
		var prodstr		=	$("#product_codename_"+k).val();
		prodstr			=	prodstr.toLowerCase();
		//alert(prodstr);
		//alert(prodstr.search(searchval));
		if(prodstr.search(searchval) != -1) {
			//alert("hi");
			//$("#prodnamebg_"+k).css({"background-color":"red"});
			$("#prodnamebg_"+k).css({"color":"#FF0000"});
			if(w == 0) {
				$("#UOM_cartons_"+k).focus();
				w++;
			}
		} else {
			$("#prodnamebg_"+k).css({"color":"#000"});
		}
	}
}

function showfullbrands(Dsrcode,propmonth,propyear) {
	//alert(Dsrcode+propmonth+propyear);
	$.ajax({
		url : "bringallbrands.php",
		type: "get",
		dataType: "text",
		data : { "DSR_Code" : Dsrcode, "propmonths" : propmonth, "propyears" : propyear },
		success : function(dataval) {
			var trimval		=	$.trim(dataval);
						
			//$("#confirmFirstMessage"+postid).css("display","none");
			$("#backgroundChatPopup").css("display","none");
			$(" <div />" ).attr("id","BrandPopup"+Dsrcode).addClass("AllBrandPop").html('<p class="closepboxa"><label class="closexbox"><a class="closelink" href="javascript:void(0)" onclick="javascript:return closeFullBrandPopup(this,\''+Dsrcode+'\');"><b><img border="0" src="../images/close_button2.png" /></b></a></label></p><p style="font-size:15px;padding-left:30px;padding-top:30px;" class="addcolor"><div id="BrandInnerPop'+Dsrcode+'" ></div></p>').appendTo($( "body" ));	
			$("#BrandPopup"+Dsrcode).css("display","block");
			$("#BrandPopup"+Dsrcode).css("z-index","120");
			$('#BrandInnerPop'+Dsrcode).html(trimval);
			$("#backgroundChatPopup").css({"opacity": "0.7"});
			$("#backgroundChatPopup").fadeIn("slow");
			return false;	
		}
...
function brandincviewajax(page,params){   // For pagination and sorting of the full brand sr incentive view page
	var splitparam		=	params.split("&");
	var DSR_Code		=	splitparam[0];
	var propyears		=	splitparam[1];
	var propmonths		=	splitparam[2];
	var sortorder		=	splitparam[3];
	var ordercol		=	splitparam[4];
	$.ajax({
		url : "bringallbrands.php",
		type: "get",
		dataType: "text",
		data : { "DSR_Code" : DSR_Code, "propyears" : propyears, "propmonths" : propmonths, "sortorder" : sortorder, "ordercol" : ordercol, "page" : page },
		success : function(dataval) {
			var trimval		=	$.trim(dataval);
			//alert(trimval);
			$("#BrandInnerPop"+DSR_Code).html(trimval);	
		}
	});
}
function closeFullBrandPopup(atr,PCode){
	$('#BrandPopup'+PCode).remove();
	$('#BrandPopup'+PCode).css('display','none');
	$('#backgroundChatPopup').fadeOut('slow');
}
function showfullproducts(Dsrcode,propmonth,propyear) {
	//alert(Dsrcode+propmonth+propyear);
	$.ajax({
		url : "bringallproducts.php",
		type: "get",
		dataType: "text",
		data : { "DSR_Code" : Dsrcode, "propmonths" : propmonth, "propyears" : propyear },
		success : function(dataval) {
			...
			//$("#confirmFirstMessage"+postid).css("display","none");
			$("#backgroundChatPopup").css("display","none");
			$(" <div />" ).attr("id","ProductPopup"+Dsrcode).addClass("AllProductPop").html('<p class="closepboxa"><label class="closexbox"><a class="closelink" href="javascript:void(0)" onclick="javascript:return closeFullProductPopup(this,\''+Dsrcode+'\');"><b><img border="0" src="../images/close_button2.png" /></b></a></label></p><p style="font-size:15px;padding-left:30px;padding-top:30px;" class="addcolor"><div id="ProductInnerPop'+Dsrcode+'" ></div></p>').appendTo($( "body" ));	
			$("#ProductPopup"+Dsrcode).css("display","block");
			$("#ProductPopup"+Dsrcode).css("z-index","120");
			$('#ProductInnerPop'+Dsrcode).html(trimval);
			$("#backgroundChatPopup").css({"opacity": "0.7"});
			$("#backgroundChatPopup").fadeIn("slow");
			return false;	
		}
...
function productincviewajax(page,params){   // For pagination and sorting of the full brand sr incentive view page
	var splitparam		=	params.split("&");
	var DSR_Code		=	splitparam[0];
	var propyears		=	splitparam[1];
	var propmonths		=	splitparam[2];
	...
	$.ajax({
		...
		success : function(dataval) {
			var trimval		=	$.trim(dataval);
			//alert(trimval);
			$("#ProductInnerPop"+DSR_Code).html(trimval);	
		}
	});
}
function closeFullProductPopup(atr,PCode){
	$('#ProductPopup'+PCode).remove();
	$('#ProductPopup'+PCode).css('display','none');
	$('#backgroundChatPopup').fadeOut('slow');
}