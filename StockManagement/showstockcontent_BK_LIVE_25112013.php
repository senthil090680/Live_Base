...
error_reporting(0);
extract($_REQUEST);
$params= $fromDate."&".$toDate."&".$sortorder."&".$ordercol."&Product_description1";
$totalval		=	0;
$totalvalue		=	0;
...
if($_REQUEST['Product_description1']!='')
{
	...
	$trimmed = trim($var);	
	$qry="SELECT * FROM `product` where Product_description1 like '%".$trimmed."%'";
}
else{
    $query_allprod="SELECT *  FROM `product`"; 
...
function productstasearch(page){  // For pagination and sorting of the SR search in view page
	var Product_description1=$("input[name='Product_description1']").val();
	$.ajax({
		url : "showstockcontentajax.php",
		type: "get",
		dataType: "text",
 ...
 <div id="search">
        <input type="text" name="Product_description1" value="<?php echo $_REQUEST['Product_description1']; ?>" autocomplete='off' placeholder='Search By Desc'/>
        <input type="button" class="buttonsg" onclick="productstasearch('<?php echo $Page; ?>');" value="GO"/>
 </div>
...
<div class="conitems">
		...
		?>
		 ...
		 <td align="center"><?php echo $priceval; ?></td>
		  <td align="center"><?php //$closingstock = intval($OpeningStock) + intval($row_receipt[recqty]) + intval($row_issue[issqty]) + intval($row_adjust[adjqty]) + intval($row_dsreturn[dsrqty]);
		  $totalvalue		+=	($priceval * $closingstock);
		 echo ($priceval * $closingstock);?></td>
		</tr>
		<?php $c++; $cc++; $y++; 
		$OpeningStock = 0;
		$closingstock = 0;
		...
		if(!empty($num_rows)){ 
			...
			rendering_pagination($Num_Pages,$Page,$Prev_Page,$Next_Page,$params,'productviewajaxsearch');   //need to uncomment
		}
		?>
		</th>
		</tr>
