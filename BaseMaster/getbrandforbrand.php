...
if($codeval	==	'' || $codeval == 'null') {
	...
	$nextrecval		=	" WHERE id IN ('".$brandcodeprint."')";
} 
...
<table align="center" width="100%">
		...
		if($rowcnt_prod > 0) {
			...
			?>
			<tr>
			<td width="731" style="width:70%"><input type="hidden" autocomplete="off" name="brandname_<?php echo $k; ?>" id="brandname_<?php echo $k; ?>" value="<?php echo $product_codearr[$k]; //echo getdbval($product_codearr[$k],'id','Product_code','product'); ?>" /><span><?php echo getdbval($product_codearr[$k],'brand','id','brand'); //echo getdbval($product_codearr[$k],'Product_description1','Product_code','product'); //PARAMETERS ARE TABLE QUERY COLUMN VALUE, TABLE RESULT COLUMN, TABLE QUERY COLUMN NAME, TABLE NAME ?></span></td>
