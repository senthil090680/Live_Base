...
if($num_rows > 0 ){
	...
	$routeyear								=	date('Y');
	$query_routemas							=	"select * from routemonthplan where DSR_Code = '$DSR_Code' AND routemonth = '$routemonth' AND $routeyear = '$routeyear'";
...
<?php echo "~"; ?>
<select class="location" name="route_sat" id="route_sat" <?php if($saturdaychecked == 0) { ?> disabled <?php } ?> style="width:100px" onChange="bringcust(this.value,'sat','route_sat');">		
	<option value="">---Select---</option>
	<?php
	$route_sql							=	"select id,location,route_desc,route_code from route_master WHERE route_code in ('".$allroutesStr."')";
	$route								=	mysql_query($route_sql) or die(mysql_error());
...
<?php echo "~".$saturdaychecked."~".$repeatchecked."~".$route_mon."~".$route_tue."~".$route_wed."~".$route_thu."~".$route_fri."~".$route_sat."~".$copyfrom; } else {
	echo "NOROUTE~"; ?>
<select class="location" name="route_mon" id="route_mon" style="width:100px" onChange="bringcust(this.value,'mon','route_mon');">
	<option value="">---Select---</option>
</select>
