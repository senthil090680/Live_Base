...
<div class="mcf"></div>
<div><h2 align="center">SR BRAND INCENTIVE TARGET VIEW</h2></div> 
<div id="containerdaily">

<span style="float:left;"><input type="button" name="kdproduct" value="Add SR Brand Incentive Tgt" class="buttonsbig" onclick="window.location='srincentive_br.php'"></span><span style="float:right;"><input type="button" name="kdproduct" value="Close" class="buttons" onclick="window.location='../include/empty.php'"></span>
...
<div class="clearfix"></div>
 <div id="search">
	...
	<input type="button" class="buttonsg" onclick="searchsrbrincviewajax('<?php echo $Page; ?>');" value="GO"/>
 </div>
 <div class="clearfix"></div>
        <?php
		if($_REQUEST['id']!=''){
			if($_REQUEST['submit']=='ConfirmDelete'){
				$Sql						=	"DELETE from srbrand_incentive WHERE id=$id";
				$delres						=	mysql_query($Sql) or die (mysql_error());
				if($delres) {
				   header("location:srincentive_br.php?no=3");
			   }
			}
		 }				
		?>
		...
		<tr>
			...
			<th class="rounded" onClick="srbrincviewajax('<?php echo $Page; ?>','<?php echo $paramsval; ?>');" >DSR Name <img src="../images/sort.png" width="13" height="13" /></th>
			<th align="center">Month & Year</th>
			<th align="center" nowrap="nowrap">Brand Name</th>
		...
		<tr>
			...
			?>
			<a href="srincentive_br.php?id=<?php echo $fetch['SRINCID'];?>"><img src="../images/user_edit.png" alt="" title="" width="11" height="11"/></a>&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<?php 
		if(!empty($num_rows)){		
			rendering_pagination_common($Num_Pages,$Page,$Prev_Page,$Next_Page,$params,'srbrincviewajax');   //need to uncomment
		} else { 
			echo "&nbsp;"; 
		} ?>      
		</th>
	  ...
	  <span id="printopen" style="padding-left:380px;padding-top:10px;<?php if($num_rows > 0 ) { echo "display:block;"; } else { echo "display:none;"; } ?>" ><input type="button" name="kdproduct" value="Print" class="buttons" onclick="print_pages('printsrbrincajax');"></span>
		<form id="printsrbrincajax" target="_blank" action="printsrbrincajax.php" method="post">
