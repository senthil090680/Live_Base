	...
	if(!isset($tgt_edit) && $tgt_edit != 'edit') {
		...
		foreach($dsrcodearrval AS $post_dsrcode) {
			for($i = 0; $i < $overall_rowcnt; $i++) {
				...
				$post_brandcode		=	$_POST["brandname_".$i];	
				...
				foreach($fromcal AS $monthyear) {
					...
					$sel_tgtcheck				=	"SELECT * from srbrand_incentive WHERE DSR_Code = '$post_dsrcode' AND monthval = '$monthval' AND yearval = '$yearval' AND Brand_id = '$post_prodcode'";
					echo "ere";
					echo $sel_tgtcheck;
					exit;
					...
					if($rowcnt_tgtcheck	> 0) {
						$update_tgt		=	"UPDATE srbrand_incentive SET KD_Code= '$KD_Code',DSR_Code = '$post_dsrcode', target_units = '$post_units', target_naira = '$post_naira',updatedatetime = NOW(), Brand_id = '$post_brandcode', targetFlag = '$post_targetflag' WHERE DSR_Code = '$post_dsrcode' AND monthval = '$monthval' AND yearval = '$yearval' AND Brand_id = '$post_brandcode'";
						$res_tgt		=	mysql_query($update_tgt);
					} else {
						echo $insert_tgt		=	"INSERT INTO srbrand_incentive (KD_Code,DSR_Code,monthval,yearval,target_units,target_naira,Brand_id,insertdatetime,targetFlag) VALUES ('$KD_Code','$post_dsrcode','$monthval','$yearval','$post_units','$post_naira','$post_brandcode',NOW(),'$post_targetflag')";						
		...
		header("location:srbrincentiveview.php?no=1");
		...
		foreach($post_dsrcodearr AS $post_dsrcode) {
			for($i = 0; $i < $overall_rowcnt; $i++) {
				...
				foreach($fromcal AS $monthyear) {
					...
					$sel_tgtcheck				=	"SELECT * from srbrand_incentive WHERE DSR_Code = '$post_dsrcode' AND monthval = '$monthval' AND yearval = '$yearval' AND Brand_id = '$post_brandcode'";
					//echo $sel_tgtcheck;
					//exit;
					$res_tgtcheck				=	mysql_query($sel_tgtcheck) or die(mysql_error());
					$rowcnt_tgtcheck			=	mysql_num_rows($res_tgtcheck);
					if($rowcnt_tgtcheck	> 0) {
						...
						$insert_tgt		=	"INSERT INTO srbrand_incentive (KD_Code,DSR_Code,monthval,yearval,target_units,target_naira,Brand_id,insertdatetime,targetFlag) VALUES ('$KD_Code','$post_dsrcode','$monthval','$yearval','$post_units','$post_naira','$post_brandcode',NOW(),'$post_targetflag')";
		...
		header("location:srbrincentiveview.php?no=2");
	}
}
...
while($row_prod 					=	mysql_fetch_array($res_prod)) {
	$product_codearr[]				=	$row_prod['id'];
}

$sel_tgtsr							=	"SELECT * from srbrand_incentive WHERE id = '$id'";
...
if($rowcnt_tgtsr	> 0) {
	...
	$sel_tgtsel						=	"SELECT * from srbrand_incentive WHERE monthval = '$tgtsrmonth' AND yearval = '$tgtsryear' AND DSR_Code = '$tgtsrDSR_Code'";
...
<div id="mainarea">
<div class="mcf"></div>
<div align="center" class="headingscredit">BRAND TARGET</div>
<div id="mytableformcredit" align="center">
<?php $curmonthval	=	ltrim(date('m'),0);
	$curyearval	=	date('Y');
?>
<form id="srincentry" method="post" action="" onSubmit="return checksrbrincentry('<?php echo $rowcnt_prod; ?>','<?php echo $curmonthval; ?>','<?php echo $curyearval; ?>');">
<div class="mcf"></div>
<div id="top">
<div style="float:center">

<fieldset class="alignment textalg">
<legend><strong>Brand Target</strong></legend>
...
<tr height="20">
	...
	<td align="left" style="width:4%" nowrap="nowrap" class="align2 alignsize">
			<span id="brandspan">
				<select class="dsrname" name="brandcode[]" id="brandcode" multiple onChange="getbrandforbrand(this.value);">
...
<span id="brandbasedbrandspan">

<table align="center" width="100%">
        <thead>
        <tr>
        <th align="center" style="width:70%">Brand</th>
		...
		if($rowcnt_tgtsel > 0) {
			...
			?>
			<tr>
			<td width="731" style="width:70%"><input type="hidden" autocomplete="off" name="brandname_<?php echo $k; ?>" id="brandname_<?php echo $k; ?>" value="<?php echo $product_codearr[$k]; ?>" /><span><?php echo getdbval($product_codearr[$k],'brand','id','brand'); //PARAMETERS ARE TABLE QUERY COLUMN VALUE, TABLE RESULT COLUMN, TABLE QUERY COLUMN NAME, TABLE NAME ?></span></td>
			...
			?>" onBlur="qtyformatreturntoid(this.value,'tgt_units_<?php echo $k; ?>');" /></td>	
			<td align="center"><input type="text" size="5" maxlength="10" autocomplete="off" name="tgt_naira_<?php echo $k; ?>" id="tgt_naira_<?php echo $k; ?>" value="<?php echo number_format($tgtsel[$k]["target_naira"],2); ?>"  onBlur="curformatreturntoid(this.value,'tgt_naira_<?php echo $k; ?>');" /></td>
			</tr>
			<?php 
			} 	//for loop ?>
...
<div id="errormsgsrinc" style="display:none;clear:both;"><h3 align="center" class="myalignsrinc"></h3><button id="closebutton_cus">Close</button></div>
 <table width="50%" style="clear:both">
	  ...
	  <input type="button" name="View" value="View" class="buttons" onclick="window.location='srbrincentiveview.php'"/>
	  </td>
      </tr>
 </table>       
 </form>
