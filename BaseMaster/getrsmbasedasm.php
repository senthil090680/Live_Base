...
if(isset($_GET[codeval]) && $_GET[codeval] !='') {
	...
	if($codevalstr != '') {
		$nextrecval		=	"WHERE RSM IN ('".$codevalstr."')";		
	}
} else {
	$nextrecval		=	"";
}
...
if(isset($_GET) && $_GET !='')
{
	$qry="SELECT * FROM `asm_sp` $where GROUP BY RSM";
}
else
{ 
	echo "Invalid Query";
...
$qry="SELECT * FROM `asm_sp` GROUP BY RSM";
$results_dsr = mysql_query($qry) or die(mysql_error());


?>

<select class="dsrname" class="s9" name="asmcode" id="asmcode" multiple <?php if($srval == 'SRNOT') { ?> onChange="getasmspecific();" <?php } else if($srval == 'SRIN') { ?> onChange="getasmspecificwithsr();" <?php } elseif($srval == 'SRINC') { ?> onChange="getasmspecificsrsta();" <?php } elseif($srval == 'SRINCE') { ?> onChange="getasmspecificsrinc();" <?php } elseif($srval == 'SRCOV') { ?> onChange="getasmspecificsrpercent();" <?php } ?> >
	<option value="">---ASM---</option>
	...
	<option value="<?php echo  $info['DSR_Code']; ?>" <?php if(!strstr($asmstr,$asmidstrval)) { if(strstr($asmstr,$info['id'])) { echo "selected"; }  } if($asmcode == $info['DSR_Code']) { echo "selected"; } ?> > <?php echo  upperstate($info['DSRName']); ?></option>
	<?php } ?> 
</select>  
<?php exit(0);?>