...
$qry_brand="SELECT *, SUM(Sold_quantity) AS SOLQTY, pr.Product_description1 AS ProdName, pr.id AS ProdId, SUM(Price) FROM `transaction_line` AS tl LEFT JOIN product pr ON tl.Product_code = pr.Product_code WHERE tl.Transaction_Number IN ('".$transno_Total."') AND tl.DSR_Code = '$DSR_Code' AND tl.Product_code != '' GROUP BY pr.Product_code";
...
if($sortorder == "")
{
	$orderby	=	"ORDER BY pr.Product_description1 ASC";
} else {
...
?>
	...
	<thead>
	<tr>
		<th align="center" colspan="19">ALL PRODUCTS TARGET INCENTIVE</th>
	 </tr>
    <tr>
	...
	<tr>
		...
		<th align="center" class="rounded" onClick="productincviewajax('<?php echo $Page; ?>','<?php echo $paramsval; ?>');" >Brand Name <img src="../images/sort.png" width="13" height="13" /></th>
		<th align="center" style="width:10%">Product Target
	...
	$ProdName					=	$fetch['ProdName'];
	$ProdId						=	$fetch['ProdId'];

	$qry_br_tgt					=	"SELECT * FROM `sr_incentive` WHERE DSR_Code = '$DSR_Code' AND monthval = '$monthval' AND yearval = '$propyears' AND Product_id = '$ProdId' ";
	//echo $qry_br_tgt;
	//exit;
	...
	<?php 
	if(!empty($num_rows)){
		rendering_pagination_common($Num_Pages,$Page,$Prev_Page,$Next_Page,$params,'productincviewajax');   //need to uncomment
	} else { 
		echo "&nbsp;"; 
	} ?>      
	</th>
  ...
  <span id="printopen" style="padding-left:380px;padding-top:10px;<?php if($num_rows > 0 ) { echo "display:block;"; } else { echo "display:none;"; } ?>" ><input type="button" name="kdproduct" value="Print" class="buttons" onclick="print_pages('printproductincajax');"></span>
	<form id="printproductincajax" target="_blank" action="printproductincajax.php" method="post">
