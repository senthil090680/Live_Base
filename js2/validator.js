...
function addamount(amtval) {
	...
	for(var k=1; k <= rowcnt; k++) {
		amt += parseInt($("#Amount_Deposited_"+k).val());
	}	
	//alert(amt);
...
function addeditamt(amtval) {
	...
	var newAmt			=	parseInt(amtval) + parseInt(balAmount);
...
function bringPopup(fromDate,toDate,Product_code,openstock,recqty,issqty,adjqty,errorid,pname) {
	...
	var insertmsg		=	'<table id="sort" border="1" class="tablesorter" width="100%"><thead><tr><th colspan="5" align="left"><h2>Stock Status</h2></th></tr><tr><th colspan="5" align="left">Product Code :   '+Product_code+' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Product Name :   ' +pname+ ' </th></tr><tr><td align="center"><strong>Opening Stock</strong></td><td align="center"><strong>Receipt</strong></td><td align="center"><strong>Issue</strong></td><td align="center"><strong>Adjustment</strong></td><td align="center"><strong>Closing Stock</strong></td></tr></tr></thead><tbody><tr><td align="center">'+openstock+'</td><td align="center"><span style="cursor:hand;cursor:pointer;color:#4285F4;" onclick="showrecords(this,\''+fromDate+'\',\''+toDate+'\',\''+Product_code+'\',\'stock_receipts\',\''+errid+'\')"> '+recqtynoneg+'</span></td><td align="center"><span style="cursor:hand;cursor:pointer;color:#4285F4;" onclick="showrecords(this,\''+fromDate+'\',\''+toDate+'\',\''+Product_code+'\',\'stock_issue\',\''+errid+'\')">'+issqtynoneg+'</span></td><td align="center"><span style="cursor:hand;cursor:pointer;color:#4285F4;" onclick="showrecords(this,\''+fromDate+'\',\''+toDate+'\',\''+Product_code+'\',\'stock_adjustment\',\''+errid+'\')">'+adjqtynoneg+'</span></td><td align="center">'+totalcnt+'</td></tr><tr><td colspan="6" align="center"><span id="'+errid+'" style="display:none;color:#ED2700;"><b>No Records Found.</b></span></td></tr></tbody></table>';		
...
function updateShortQty(P_code){
	...
	if(KD_returned_qty == ''){
		...
		$('#qtyshotspan_'+P_code).html(quantity_shortage);
...
function checkprodconfirm() {
	...
	for(var k=1; k <= prodcnt; k++) {
		if($('#cbox_'+k).is(":checked")) {
			...
			var Loaded_Qty			=	parseInt($.trim($("#Loaded_Qty_"+k).val()));
			var product_code_val	=	$("#product_code_"+k).val();
			//alert(actual_qty);
			//alert(Loaded_Qty);
			if(Loaded_Qty == ''){
...
function changetotalval(rowcnt) {
	var valueval	=	0;
	for(var k=1; k<=rowcnt; k++ ) {
		valuevalue	=	parseInt($("#priceval_"+k).val()) * parseInt($("#returnqty_"+k).val());
		$("#valueval_"+k).val(valuevalue);
		$("#valuevalspan_"+k).html(valuevalue);		
		valueval	+=	valuevalue;
	}
	//alert(valueval);
	$("#totvalspan").html(valueval);
	$("#totval").val(valueval);
