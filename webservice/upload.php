...
if(!file_exists($cronlogpath."/".$directory."/".$current_datealone."_croncheck.txt")) {
//echo "good";
//exit;
	$filemode			=	fopen($cronlogpath."/".$directory."/".$current_datealone."_croncheck.txt","a+");
	chmod($cronlogpath."/".$directory."/".$current_datealone."_croncheck.txt", 0777);
...
//$directory					=	$uploadfolderpath."/Upload/";				//WHEN LIVE REMOVE COMMENT
//$uploadedDirectory			=	$uploadfolderpath."/UploadedFiles/";		//WHEN LIVE REMOVE COMMENT
			...
			if ($index == 1) {
				...
				if($rowcnt_transhdr == 0){
					...
					if($new_Customer_Code_Created_Arr[$lineproductarr[6]] == '' || !isset($new_Customer_Code_Created_Arr[$lineproductarr[6]])) {
						...
						$lineproductarr[22]			=	getdbval($lineproductarr[1], 'RSM', 'DSR_Code', 'dsr');	
						
						$lineproductarr[23]			=	getdbval($lineproductarr[22], 'branch_id', 'id', 'rsm_sp');
						...
						$result		=	mysql_query($query) or die(mysql_error()." At Line : ". __LINE__);
						//echo $query;
						//exit;				
